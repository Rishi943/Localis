  ...
  <action>
    ...
    function _startOnboarding() {
        ...
        skipBtn?.addEventListener('click', async () => {
            // Mark as done so skip persists across reopens
            try { await fetch('/finance/goals', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({goal_type:'skipped',life_events:[],budgets:{},horizon:''}); } } catch(e) {}
            document.querySelectorAll('.fin-tab').forEach(t => t.style.display = '');
            _activateTab('dashboard');
            _loadDashboard(_currentPeriod);
        });
    ...
    Note: There is a syntax error in the skip button fetch call in the pseudocode above — the body closing must be correct JSON.stringify. Fix: `body: JSON.stringify({goal_type:'skipped',life_events:[],budgets:{},horizon:''})` (remove the stray semicolon inside the object literal).
  </action>
  <verify>
    <automated>python3 -c "
js = open('/home/rishi/Rishi/AI/Localis/app/static/js/app.js').read()
...
"</automated>
  ...
  <done>
    _startOnboarding() runs the 4-step conversation. Budget step renders inline form. On complete, POST /finance/goals saves answers and transitions to Dashboard tab. Skip button exits to Dashboard without saving.
  </done>
</task>
